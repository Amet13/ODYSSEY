...
final class ValidationService {
  ...
  private let logger = Logger(
    subsystem: AppConstants.loggingSubsystem, category: "ValidationService")
  ...
  func validateEmail(_ email: String) -> Bool {
    ...
    let emailRegex =
      AppConstants.patterns["email"] ?? "^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$"
  ...
  func validateGmailAppPassword(_ password: String) -> Bool {
    ...
    let pattern =
      AppConstants.patterns["gmailAppPassword"] ?? "^[a-z]{4}\\s[a-z]{4}\\s[a-z]{4}\\s[a-z]{4}$"
  ...
  func validateFacilityURL(_ url: String) -> Bool {
    ...
    let pattern =
      AppConstants.patterns["facilityURL"]
