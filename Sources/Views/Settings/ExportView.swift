...
struct ExportView: View {
  ...
  private func generateExportToken() async {
    ...
    errorMessage = nil
    do {
      let token = try await cliExportService.exportForCLI(
        selectedConfigIds: Array(selectedConfigIds))
      await MainActor.run {
        exportToken = token
        isExporting = false
      ...
      HStack {
        ...
        VStack(alignment: .leading, spacing: AppConstants.spacingTiny) {
          Text(config.name)
            ...
            .fontWeight(.medium)
          Text(
            "\(config.sportName) • \(ReservationConfig.extractFacilityName(from: config.facilityURL))"
          )
