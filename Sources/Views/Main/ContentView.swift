...
private struct EmptyStateView: View {
  ...
  var body: some View {
    VStack(spacing: AppConstants.sectionSpacing) {
      ...
      Text(NSLocalizedString("empty_state_title", comment: "Empty state title"))
        .font(.system(size: AppConstants.fontTitle3))
        .fontWeight(.semibold)
        .accessibilityAddTraits(.isHeader)
      Text(NSLocalizedString("empty_state_subtitle", comment: "Empty state subtitle"))
        .font(.system(size: AppConstants.fontSubheadline))
        .foregroundColor(Color.odysseySecondaryText)
        ...
        if let next = nextAutorunInfo {
          HStack(spacing: AppConstants.spacingTiny) {
            Image(systemName: AppConstants.SFSymbols.clock)
              ...
              .font(.system(size: AppConstants.fontSubheadline))
              .foregroundColor(.odysseyAccent)
            Text(NSLocalizedString("next_autorun_in", comment: "Next autorun label"))
              .font(.system(size: AppConstants.fontSubheadline))
              .foregroundColor(.odysseyAccent)
      ...
      return AnyView(
        HStack(spacing: AppConstants.spacingTiny) {
          Image(systemName: statusInfo.iconName)
            ...
            .font(.system(size: AppConstants.fontSubheadline))
          Text(NSLocalizedString("last_run", comment: "Last run label"))
            .font(.system(size: AppConstants.fontSubheadline))
            .foregroundColor(statusInfo.statusColor)
