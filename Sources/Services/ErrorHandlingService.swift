...
public final class ErrorHandlingService: ObservableObject {
  ...
  private let logger = Logger(
    subsystem: AppConstants.loggingSubsystem, category: "ErrorHandlingService")
  ...
  public nonisolated func logError(
    _ message: String, error: Error? = nil, context: String = "General"
  ) {
    let fullMessage =
      error != nil ? "\(message): \(error?.localizedDescription ?? "Unknown error")" : message
