...
public enum WebDriverError: Error, LocalizedError, UnifiedErrorProtocol {
  ...
  public var errorCode: String {
    ...
    case .navigationFailed:
      return AppConstants.errorCodes["navigationFailed"] ?? "WEBDRIVER_NAVIGATION_001"
    case .elementNotFound:
      return AppConstants.errorCodes["elementNotFound"] ?? "WEBDRIVER_ELEMENT_001"
    ...
    case .scriptExecutionFailed:
      return AppConstants.errorCodes["scriptExecutionFailed"] ?? "WEBDRIVER_SCRIPT_001"
    case .timeout: return AppConstants.errorCodes["timeout"] ?? "WEBDRIVER_TIMEOUT_001"
    case .connectionFailed:
      return AppConstants.errorCodes["connectionFailed"] ?? "WEBDRIVER_CONNECTION_001"
    case .invalidSelector:
      return AppConstants.errorCodes["invalidSelector"] ?? "WEBDRIVER_SELECTOR_001"
...
public struct Configuration: Equatable {
  ...
  public init(
    id: UUID = UUID(), name: String, facilityURL: String, sportName: String, isEnabled: Bool = true
  ) {
    self.id = id
    self.name = name
