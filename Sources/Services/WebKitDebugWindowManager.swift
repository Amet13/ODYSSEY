...
public final class WebKitDebugWindowManager: NSObject, NSWindowDelegate {
  ...
  public func showDebugWindow(webView: WKWebView?, config: ReservationConfig?) {
    ...
    if !userSettings.showBrowserWindow {
      logger.info(
        "🪟 Debug window hidden (user setting: hide window - recommended to avoid captcha detection)."
      )
      return
    }
...
extension WebKitDebugWindowManager {
  /// Configuration for debug window behavior
  public struct DebugWindowConfig {
    ...
    public init(
      ...
      windowSize: CGSize = CGSize(
        width: AppConstants.webKitWindowWidth, height: AppConstants.webKitWindowHeight),
      windowPosition: CGPoint = CGPoint(
        x: AppConstants.webKitDebugWindowX, y: AppConstants.webKitDebugWindowY),
