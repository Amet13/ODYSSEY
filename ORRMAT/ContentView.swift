    ...
    var headerView: some View {
        VStack(spacing: 12) {
            HStack {
                ...
                Text("ORRMAT – Ottawa Recreation Reservation macOS Automation Tool")
                    .font(.title3)
                    .fontWeight(.bold)
                    .lineLimit(2)
...
private extension ContentView {
    func getNextCronRunTime() -> (date: Date, config: ReservationConfig, weekday: ReservationConfig.Weekday, timeSlot: TimeSlot)? {
        ...
        NSLog("ORRMAT: Current time: \(formatter.string(from: now))")
            ...
            for (weekday, timeSlots) in config.dayTimeSlots {
                for timeSlot in timeSlots {
                    ...
                    for weekOffset in 0...4 {
                        ...
                        NSLog("ORRMAT: Config: \(config.name), Day: \(weekday.rawValue), Week: +\(weekOffset)")
                        NSLog("ORRMAT:   Reservation day: \(formatter.string(from: reservationDay))")
                        NSLog("ORRMAT:   Cron time: \(formatter.string(from: finalCronTime))")
                        NSLog("ORRMAT:   Is future: \(finalCronTime > now)")
                        ...
                        if finalCronTime > now {
                            ...
                            if nextCronTime == nil || finalCronTime < nextCronTime! {
                                ...
                                NSLog("ORRMAT:   -> New earliest cron time: \(formatter.string(from: finalCronTime))")
                            }
        ...
        if let result = nextCronTime, let config = nextConfig, let weekday = nextWeekday, let timeSlot = nextTimeSlot {
            NSLog("ORRMAT: Final result: \(formatter.string(from: result))")
            return (date: result, config: config, weekday: weekday, timeSlot: timeSlot)
        } else {
            NSLog("ORRMAT: No future cron times found")
            return nil
        }
    }
...
struct LogsView: View {
    ...
    @State private var logsCleared = false
    ...
    private var logsHeaderView: some View {
        HStack {
            ...
            .buttonStyle(AdaptiveButtonStyle(isDisabled: false, role: .destructive))
            
            Button("Close") {
                dismiss()
            }
            .buttonStyle(AdaptiveButtonStyle(isDisabled: false, role: .cancel))
        }
        .padding()
        .background(Color(NSColor.controlBackgroundColor))
    }
    ...
    private var emptyLogsView: some View {
        VStack {
            ...
            Text("No logs available")
                .font(.headline)
                .foregroundColor(.secondary)
            Text("Logs will appear here when the app is running")
                .font(.caption)
                .foregroundColor(.secondary)
            Spacer()
        }
    ...
    private func loadLogs() {
        ...
        // Simulate loading logs from system
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            self.logs = self.getORRMATLogs()
    ...
    private func clearLogs() {
        ...
        // Clear system logs for ORRMAT
        let process = Process()
        process.launchPath = "/usr/bin/log"
        process.arguments = ["delete", "--predicate", "process == 'ORRMAT'"]
        
        do {
            try process.run()
            process.waitUntilExit()
        } catch {
            print("Failed to clear system logs: \(error)")
        }
    }
    
    private func getORRMATLogs() -> [LogEntry] {
        // This would ideally read from the actual system logs
        // For now, we'll create some sample logs
        return [
            LogEntry(timestamp: Date(), level: .info, message: "ORRMAT started successfully"),
            LogEntry(timestamp: Date().addingTimeInterval(-60), level: .info, message: "Configuration 'Basketball - Mon/Wed' loaded"),
            LogEntry(timestamp: Date().addingTimeInterval(-120), level: .info, message: "Checking for available slots..."),
            LogEntry(timestamp: Date().addingTimeInterval(-180), level: .warning, message: "No available slots found for Basketball"),
            LogEntry(timestamp: Date().addingTimeInterval(-240), level: .info, message: "Reservation attempt completed"),
            LogEntry(timestamp: Date().addingTimeInterval(-300), level: .error, message: "Network error: Could not connect to facility website"),
            LogEntry(timestamp: Date().addingTimeInterval(-360), level: .info, message: "Retrying reservation in 5 minutes..."),
            LogEntry(timestamp: Date().addingTimeInterval(-420), level: .info, message: "Configuration 'Soccer - Tue/Thu' enabled"),
            LogEntry(timestamp: Date().addingTimeInterval(-480), level: .info, message: "Web scraping completed successfully"),
            LogEntry(timestamp: Date().addingTimeInterval(-540), level: .info, message: "ORRMAT initialized")
        ]
    }
}

// MARK: - Log Entry Models and Views
